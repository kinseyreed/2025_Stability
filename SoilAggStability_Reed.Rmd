...
plot_gn <- ggplot(data = data_filt, aes(x = gram_negative, y = stability, color = as.factor(year), size = om_perc)) +
  ...
  annotate("text", x = 35, y = 1.9, label = gn_label_all,    color = "black",  size = 3.5) +
...
var_labels <- c(
  ...
  total_plfa   = "Total PLFAs (nmol per g soil)",
...
# Extract the aligned model for the interaction
...
# Extract F values and p-values for annotation
...
# Create mapping between original and renamed PLFA groups
plfa_mapping <- c(
...
# Function to automatically assign letters based on contrast results
assign_letters_auto <- function(contrasts_df, year_val, plot_order, alpha = 0.05) {
  # Extract contrasts for the specific year
  year_contrasts <- contrasts_df %>% 
    filter(year == year_val) %>%
    mutate(contrast = as.character(contrast))
  
  if(nrow(year_contrasts) == 0) return(data.frame())
  
  # Print contrasts for this year for debugging
  cat("\n=== Contrasts for", year_val, "===\n")
  debug_contrasts <- year_contrasts %>%
    select(contrast, p.value) %>%
    mutate(significant = p.value < alpha)
  print(debug_contrasts)
  
  # Get all unique groups
  all_groups <- unique(c(
    sapply(strsplit(year_contrasts$contrast, " - "), function(x) trimws(x[1])),
    sapply(strsplit(year_contrasts$contrast, " - "), function(x) trimws(x[2]))
  ))
  all_groups <- sort(all_groups[!is.na(all_groups)])
  n_groups <- length(all_groups)
  
  # Create adjacency matrix (TRUE = NOT significantly different)
  adj_matrix <- matrix(TRUE, nrow = n_groups, ncol = n_groups)
  rownames(adj_matrix) <- colnames(adj_matrix) <- all_groups
  
  # Fill in significance relationships
  for(i in 1:nrow(year_contrasts)) {
    contrast_parts <- strsplit(year_contrasts$contrast[i], " - ")[[1]]
    group1 <- trimws(contrast_parts[1])
    group2 <- trimws(contrast_parts[2])
    p_val <- year_contrasts$p.value[i]
    
    # If significantly different, set to FALSE
    if(p_val < alpha) {
      adj_matrix[group1, group2] <- FALSE
      adj_matrix[group2, group1] <- FALSE
    }
  }
  
  # Map plot names back to original names for ordering
  name_mapping <- c(
    "Gram-Negative\n Bacteria" = "gram_negative",
    "Gram-Positive\n Bacteria" = "gram_positive", 
    "Actinobacteria" = "actinomycetes",
  ...
  # Get the plot order in original names
  original_plot_order <- name_mapping[plot_order]
  original_plot_order <- original_plot_order[!is.na(original_plot_order)]
  
  # Available letters
  available_letters <- if(year_val == "2022") LETTERS else letters
  
  # Initialize group letters
  group_letters <- setNames(rep(NA, length(all_groups)), all_groups)
  
  # Find connected components (groups that can share letters)
  connected_components <- list()
  visited <- setNames(rep(FALSE, length(all_groups)), all_groups)
  
  # Use depth-first search to find connected components
  dfs <- function(node, component) {
    visited[node] <<- TRUE
    component <- c(component, node)
    
    for(neighbor in all_groups) {
      if(!visited[neighbor] && adj_matrix[node, neighbor]) {
  ...
  for(group in all_groups) {
    if(!visited[group]) {
      component <- dfs(group, character(0))
      connected_components[[length(connected_components) + 1]] <- component
    }
  }
  
  cat("Connected components (groups that can share letters):\n")
  for(i in 1:length(connected_components)) {
    cat("Component", i, ":", paste(connected_components[[i]], collapse = ", "), "\n")
  }
  
  # Assign letters to components in plot order
  letter_index <- 1
  
  for(group in original_plot_order) {
    if(group %in% all_groups && is.na(group_letters[group])) {
      
      # Find which component this group belongs to
      component_idx <- which(sapply(connected_components, function(comp) group %in% comp))
      
      if(length(component_idx) > 0) {
        current_component <- connected_components[[component_idx[1]]]
        
        # Assign the same letter to all groups in this component
        current_letter <- available_letters[letter_index]
        for(comp_group in current_component) {
          group_letters[comp_group] <- current_letter
        }
  ...
  # Print letter assignments for debugging
  cat("Letter assignments for", year_val, ":\n")
  for(i in 1:length(group_letters)) {
    cat(names(group_letters)[i], "->", group_letters[i], "\n")
  }
  
  # Create result dataframe
  letter_df <- data.frame(
    PLFA_Group_original = names(group_letters),
    year = year_val,
    letter = as.character(group_letters),
...
# Function to automatically assign stars based on year-within-group contrasts
assign_stars_auto <- function(contrasts_df, alpha = 0.05) {
  # Print all year contrasts for debugging
  cat("\n=== Year-within-group contrasts ===\n")
  debug_year_contrasts <- contrasts_df %>%
    select(PLFA_Group, p.value) %>%
    mutate(
      significant = p.value < alpha,
      stars = get_stars(p.value)
    )
  print(debug_year_contrasts)
  
  # Create significance data with CORRECT mapping
  signif_data <- contrasts_df %>%
    mutate(stars = get_stars(p.value)) %>%
    filter(stars != "ns") %>%
    mutate(
      PLFA_Group_original = PLFA_Group,
      PLFA_Group = plfa_mapping[PLFA_Group]  # This should map correctly now
    ) %>%
    select(PLFA_Group_original, PLFA_Group, p.value, stars)
  
  if(nrow(signif_data) > 0) {
    cat("\nGroups with significant year effects (CORRECTED):\n")
    print(signif_data[, c("PLFA_Group_original", "PLFA_Group", "p.value", "stars")])
  } else {
    cat("\nNo significant year effects found.\n")
  }
...
# Get the plot order
plot_order <- levels(boxplot.cleaned$PLFA_Group)
cat("Plot order (left to right):", paste(plot_order, collapse = " -> "), "\n")

# Generate letters and stars automatically based on contrast results
letters_2022 <- assign_letters_auto(df_group_within_year, "2022", plot_order)
letters_2023 <- assign_letters_auto(df_group_within_year, "2023", plot_order)
letter_data <- rbind(letters_2022, letters_2023)

# Add renamed PLFA groups to letter data
letter_data <- letter_data %>%
  mutate(PLFA_Group = plfa_mapping[PLFA_Group_original])

# Generate stars automatically
signif_data <- assign_stars_auto(df_year_within_group)
...
y_positions <- boxplot.cleaned %>%
  ...
  mutate(
    y_pos_stars = y_max + 0.20 * max(y_max),
    y_pos_letters = y_max + 0.08 * max(y_max)
  )
...
# Separate by year and add positions
letters_2022_plot <- letter_data %>% 
  filter(year == "2022") %>%
  mutate(x_position = match(PLFA_Group, plot_order) - 0.2)

letters_2023_plot <- letter_data %>% 
  filter(year == "2023") %>%
  mutate(x_position = match(PLFA_Group, plot_order) + 0.2)
...
# Add stars for significant year effects
if(nrow(signif_data) > 0) {
  plfa_plot <- plfa_plot +
    geom_text(data = signif_data, 
              aes(x = PLFA_Group, y = y_pos_stars, label = stars),
              inherit.aes = FALSE,
              size = 5, vjust = 0.5, fontface = "bold")
}

# Add letters for group differences within years
if(nrow(letters_2022_plot) > 0) {
  plfa_plot <- plfa_plot +
    geom_text(data = letters_2022_plot,
              aes(x = x_position, y = y_pos_letters, label = letter),
              inherit.aes = FALSE,
              size = 4, fontface = "bold", color = "#1f77b4")
}

if(nrow(letters_2023_plot) > 0) {
  plfa_plot <- plfa_plot +
    geom_text(data = letters_2023_plot,
              aes(x = x_position, y = y_pos_letters, label = letter),
              inherit.aes = FALSE,
              size = 4, fontface = "bold", color = "darkorange")
}
...
ggsave(filename = "fig1patchwork_col_fullinter.png", plot = Figure1_All)

ggsave(filename = "fig1patchwork_col_full_inter.tiff", plot = Figure1_All)

ggsave(filename = "fig1patchwork_col_full_inter.pdf", plot = Figure1_All)
#ggsave(filename = "fig1patchwork_col_full_inter.eps", plot = Figure1_All)
#% increases for text
...
#fit the nonparametric transformation 
art_model_prop <- art(prop_abundance ~ PLFA_Group * year + (1 | anon_id), data = boxplot.cleaned)

# 2. Extract the aligned model for the interaction
art_interaction_prop <- artlm(art_model_prop, "PLFA_Group:year")
#singular error ok

# Post-hoc: all PLFA_Group × year comparisons
emms_full_p <- emmeans(art_interaction_prop, ~ PLFA_Group * year)
pairs_full_p <- contrast(emms_full_p, method = "pairwise", adjust = "fdr")

# PLFA Group comparisons within each year
pairs_group_within_year_p <- emmeans(art_interaction_prop, pairwise ~ PLFA_Group | year, adjust = "fdr")

# year comparisons within each PLFA Group
pairs_year_within_group_p <- emmeans(art_interaction_prop, pairwise ~ year | PLFA_Group, adjust = "fdr")


# Convert to data frames
df_full_p <- as.data.frame(pairs_full_p)
df_group_within_year_p <- as.data.frame(pairs_group_within_year$contrasts)
df_year_within_group_p <- as.data.frame(pairs_year_within_group$contrasts)

df_full_p$Signif <- get_stars(df_full_p$p.value)
df_group_within_year_p$Signif <- get_stars(df_group_within_year_p$p.value)
df_year_within_group_p$Signif <- get_stars(df_year_within_group_p$p.value)


# Export to CSV
write.csv(df_full_p, "PostHoc_Full_Interaction_Proportional.csv", row.names = FALSE)
write.csv(df_group_within_year_p, "PostHoc_Within_year_By_Group_Proportional.csv", row.names = FALSE)
write.csv(df_year_within_group_p, "PostHoc_Within_Group_By_year_Proportional.csv", row.names = FALSE)


#View results
summary(art_model_prop)
anova_res_p<- anova(art_model_prop) #all sig 
anova_res_p
#Group ***, year NS, Interaction ***

write.csv(anova_res_p, "ART_anova_result_proportional.csv")

# Extract F values and p-values
f_vals_p <- round(anova_res_p$F, 2)
p_vals_p <- anova_res_p$`Pr(>F)`

# Compose annotation text
annotation_text_p <- paste0(
  #"Proportional PLFA ~ PLFA Group * Year + (1 | SiteID)", "\n",
  "Group: F=", f_vals_p[1], stars[1], "\n",
  "Year: F=", f_vals_p[2], stars[2], "\n",
  "Group × Year: F=", f_vals_p[3], stars[3]
)


#reorder based on descending value (now using proportional abundance)
boxplot.cleaned <- boxplot.cleaned %>%
  mutate(PLFA_Group = fct_reorder(
    PLFA_Group,
    prop_abundance,
...
# Create a mapping between original and renamed PLFA groups
#plfa_mapping <- c(
 # "actinomycetes" = "Actinobacteria",
  #"gram_negative" = "Gram-Negative\n Bacteria", 
  #"am_fungi" = "AM Fungi",
...
# Improved function to assign compact letter display based on actual contrast data
assign_letters_from_contrasts <- function(contrasts_df, year_val, plot_order, alpha = 0.05) {
  # Extract contrasts for the specific year
  year_contrasts <- contrasts_df %>% 
    filter(year == year_val) %>%
    mutate(contrast = as.character(contrast))
  ...
  rownames(sig_matrix) <- colnames(sig_matrix) <- all_groups
  ...
  for(i in 1:nrow(year_contrasts)) {
    ...
    # If significantly different (p < alpha), set to TRUE
    if(p_val < alpha) {
      sig_matrix[group1, group2] <- TRUE
      sig_matrix[group2, group1] <- TRUE
    }
  }
  
  # Map original names to plot names for ordering
  name_mapping <- c(
    "actinomycetes" = "Actinobacteria",
    "gram_negative" = "Gram-Negative\n Bacteria", 
    "am_fungi" = "AM Fungi",
  ...
  # Get the plot order and map back to original names
  original_plot_order <- names(name_mapping)[match(plot_order, name_mapping)]
  ...
  # Assign letters in plot order (left to right)
  letter_index <- 1
  
  for(group in original_plot_order) {
    if(group %in% all_groups && is.na(group_letters[group])) {
      # Find which letters are forbidden (used by significantly different groups)
      forbidden_letters <- character(0)
      
      for(other_group in all_groups) {
        if(sig_matrix[group, other_group] && !is.na(group_letters[other_group])) {
          forbidden_letters <- c(forbidden_letters, group_letters[other_group])
        }
      }
      
      # Find the first available letter in alphabetical order
      current_letter <- available_letters[letter_index]
      while(current_letter %in% forbidden_letters && letter_index <= length(available_letters)) {
        letter_index <- letter_index + 1
        if(letter_index <= length(available_letters)) {
          current_letter <- available_letters[letter_index]
        }
      }
      
      # Assign this letter to the current group
      group_letters[group] <- current_letter
      
      # Also assign to all non-significantly different groups that don't have letters yet
      for(other_group in all_groups) {
        if(!sig_matrix[group, other_group] && is.na(group_letters[other_group])) {
          group_letters[other_group] <- current_letter
        }
      }
...
# Get the plot order first
plot_order <- levels(boxplot.cleaned$PLFA_Group)

# Generate letters for both years using the corrected function with plot order
letters_2022_p <- assign_letters_from_contrasts(df_group_within_year_p, "2022", plot_order)
letters_2023_p <- assign_letters_from_contrasts(df_group_within_year_p, "2023", plot_order)
letter_data_p <- rbind(letters_2022_p, letters_2023_p)

# Add renamed PLFA groups to letter data
letter_data_p <- letter_data_p %>%
  mutate(PLFA_Group = plfa_mapping[PLFA_Group_original])

cat("Letter assignments (ordered by plot position):\n")
print(letter_data)

# Prepare significance data from ART-ANOVA contrasts (year within group)
# Based on the actual df_year_within_group data you provided
signif_data_p <- data.frame(
  PLFA_Group_original = c("eukaryote", "gram_negative", "gram_positive"),
  PLFA_Group = c("Eukaryote", "Gram-Negative\n Bacteria", "Gram-Positive\n Bacteria"),
  p.value = c(0.003402139, 0.004062614, 0.002200239),
  stars = c("**", "**", "**"),
  stringsAsFactors = FALSE
)

cat("Corrected Year-within-group significance results:\n")
print(signif_data_p)

# Calculate y positions for different labels (now using proportional abundance)
y_positions_p <- boxplot.cleaned %>%
  group_by(PLFA_Group) %>%
  summarise(y_max = max(prop_abundance, na.rm = TRUE)) %>%
  mutate(
    y_pos_stars = y_max + 0.20 * max(y_max), # Stars on top
    y_pos_letters = y_max + 0.08 * max(y_max) # Letters closer to boxes
  )

# Merge significance data with y positions (only once)
if(nrow(signif_data_p) > 0) {
  signif_data_p <- signif_data_p %>%
    left_join(y_positions_p, by = "PLFA_Group")
  
  cat("\nCorrected final signif_data:\n")
  print(signif_data_p[, c("PLFA_Group_original", "PLFA_Group", "p.value", "stars")])
}

# Add positions to letter data and create separate datasets for each year
letter_data_p <- letter_data_p %>%
  left_join(y_positions_p, by = "PLFA_Group")

letters_2022_plot_p <- letter_data_p %>% filter(year == "2022")
letters_2023_plot_p <- letter_data_p %>% filter(year == "2023")

# Get the actual order of PLFA groups in the plot (after reordering)
plot_order_p <- levels(boxplot.cleaned$PLFA_Group)

# Create position mapping for letters
letters_2022_plot_p <- letters_2022_plot_p %>%
  mutate(x_position = match(PLFA_Group, plot_order) - 0.2)

letters_2023_plot_p <- letters_2023_plot_p %>%
  mutate(x_position = match(PLFA_Group, plot_order) + 0.2)

#Fig 1 Plot D - Now showing proportional abundance
...
# Add significance labels from ART-ANOVA contrasts
# Stars for year-within-group comparisons
if(nrow(signif_data_p) > 0) {
  plfa_plot_p <- plfa_plot_p +
    geom_text(data = signif_data_p, 
              aes(x = PLFA_Group, y = y_pos_stars, label = stars),
              inherit.aes = FALSE,
              size = 5, vjust = 0.5, fontface = "bold")
}

# Letters for group-within-year comparisons
# Add 2022 letters (uppercase) positioned over 2022 boxes
if(nrow(letters_2022_plot_p) > 0) {
  plfa_plot_p <- plfa_plot_p +
    geom_text(data = letters_2022_plot_p,
              aes(x = x_position, y = y_pos_letters, label = letter),
              inherit.aes = FALSE,
              size = 4, fontface = "bold", color = "#1f77b4")
}

# Add 2023 letters (lowercase) positioned over 2023 boxes  
if(nrow(letters_2023_plot_p) > 0) {
  plfa_plot_p <- plfa_plot_p +
    geom_text(data = letters_2023_plot_p,
              aes(x = x_position, y = y_pos_letters, label = letter),
              inherit.aes = FALSE,
              size = 4, fontface = "bold", color = "darkorange")
}
