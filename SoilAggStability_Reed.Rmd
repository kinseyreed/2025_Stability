...
# Fit the nonparametric model using proportional abundance
# Try without random effect first to see if that resolves the singular fit
#gave error previously suggesting random effect did not contribute to the model. singular fit warning
#art_model <- art(prop_abundance ~ PLFA_Group * year, data = boxplot.cleaned)

# If you want to keep the random effect but address the warning, you can try:
# Option 1: Use a simpler random effect structure
 #art_model <- art(prop_abundance ~ PLFA_Group * year + (1 | anon_id), data = boxplot.cleaned, control = lmerControl(check.conv.singular = "ignore"))

# Option 2: Check if the random effect is needed by comparing models
# You can uncomment these lines to compare models:
#art_model_no_random <- art(prop_abundance ~ PLFA_Group * year, data = boxplot.cleaned)
art_model <- art(prop_abundance ~ PLFA_Group * year + (1 | anon_id), data = boxplot.cleaned)
#anova(artlm(art_model_no_random, "PLFA_Group:year"), artlm(art_model_with_random, "PLFA_Group:year")) 
...
# Export to CSV
write.csv(df_full, "PostHoc_Full_Interaction_Proportional.csv", row.names = FALSE)
write.csv(df_group_within_year, "PostHoc_Within_year_By_Group_Proportional.csv", row.names = FALSE)
write.csv(df_year_within_group, "PostHoc_Within_Group_By_year_Proportional.csv", row.names = FALSE)
...
write.csv(anova_res, "ART_anova_result_proportional_withrandom.csv")
...
letters_2022 <- assign_letters_from_contrasts(df_group_within_year, "2022", plot_order)
letters_2023 <- assign_letters_from_contrasts(df_group_within_year, "2023", plot_order)
letter_data <- rbind(letters_2022, letters_2023)
...
letters_2022_plot <- letter_data %>% filter(year == "2022")
letters_2023_plot <- letter_data %>% filter(year == "2023")

# Get the actual order of PLFA groups in the plot (after reordering)
plot_order <- levels(boxplot.cleaned$PLFA_Group)

# Create position mapping for letters
letters_2022_plot <- letters_2022_plot %>%
  mutate(x_position = match(PLFA_Group, plot_order) - 0.2)

letters_2023_plot <- letters_2023_plot %>%
  mutate(x_position = match(PLFA_Group, plot_order) + 0.2)
